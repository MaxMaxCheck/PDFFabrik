...
export const useFileUpload = () => {
  ...
  if (!context) {
    throw new Error("useFileUpload must be used within a FileUploader provider")
  }
  return context
}
      ...
      >
        <div
          ...
          className={cn(
            ...
            className,
            value && value.length > 0 ? "gap-2" : null
          )}
          dir={dir}
          {...props}
        >
...
type FileInputProps = React.HTMLAttributes<HTMLDivElement> & {
  /** Keine innere Border/Rounded-Frame (wenn ein Outer-Shell das schon macht). */
  frameless?: boolean
}

export const FileInput = forwardRef<HTMLDivElement, FileInputProps>(
  ({ className, children, frameless, ...props }, ref) => {
    ...
    return (
      <div
        ...
        className={cn(
          ...
          frameless && "flex min-h-0 min-w-0 flex-1 flex-col"
        )}
      >
        <div
          className={cn(
            frameless
              ? "flex min-h-0 w-full flex-1 flex-col border-0 border-transparent bg-transparent shadow-none outline-none ring-0"
